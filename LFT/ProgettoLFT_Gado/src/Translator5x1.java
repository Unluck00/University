    ...
    public void prog() {
        ...
        switch (look.tag) {
            case Tag.ASSIGN, Tag.PRINT, Tag.READ, Tag.FOR, Tag.IF, '{' -> {
                statlist(lnext_prog);
                code.emitLabel(lnext_prog); // creazione di una label (L0) per JMP
                match(Tag.EOF);
                try {
                    code.toJasmin();
                }
    ...
    private void stat(int next) {
        switch (look.tag) {
            ...
            case Tag.READ -> {
                ...
                match(Tag.READ);
                match(40); // 40 è ( in ASCII
                idlist(1, value); // passa 1 per identificare la read (0 per identificare la assignlist); mentre value è un valore arbitrario (inutile) che serve solo per restituire il valore numerico da expr()
                match(41); // 40 è ) in ASCII
            }
            ...
            case Tag.FOR -> {
                ...
                code.emitLabel(for_true); // creazione di una label in questo punto per JMP

                stat(next);

                code.emit(OpCode.GOto, stat_next); // aggiunta di un istruzione GOto che indica un JMP presso la label stat_next
            }
