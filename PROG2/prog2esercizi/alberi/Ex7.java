/* 
Salvate tutto in un solo file Ex7.java.
Completate il metodo Tree <T> g(T x) in Leaf <T> e Branch <T> come segue.

1) g(x) raddoppia tutti i nodi il cui elemento è maggiore di x, aggiungendo un nuovo nodo identico come figlio sinistro.
2) Se l'elemento di un nodo non è maggiore di x, il metodo lascia il nodo invariato e procede ricorsivamente sui sottoalberi.
...
Eseguite il main della classe Ex7 come test per la risposta. Non definite il vostro metodo per casi sui test.
*/

import java.util.*;

abstract class Tree<T extends Comparable<T>> {
    public abstract Tree<T> g(T x);
...
class Ex7 {
    ...
    public static Tree<T> d(T elem, Tree<T> left, Tree<T> right) {
        if (elem.compareTo(x) > 0) {
            return new Branch<T>(elem, new Branch<T>(elem, left, new Leaf<T>()), right);
        } else {
            return new Branch<T>(elem, left, right);
        }
    }
    ...
    public static void test() {
        ...
        check("t1 = " + t1.toString() + "\n t1.g(b)=", t1.g(x).toString(), ris1.toString());
        check("t2 = " + t2.toString() + "\n t2.g(b)=", t2.g(x).toString(), ris2.toString());
    }
    ...
    static void msg() {
        ...
        System.out.println("g raddoppia i nodi con elemento maggiore di b");
    }
...
public Tree<T> g(T x) {
    if (elem.compareTo(x) > 0) {
        left = new Branch<T>(elem, left, new Leaf<T>());
    }
    left = left.g(x);
    right = right.g(x);
    return this;
