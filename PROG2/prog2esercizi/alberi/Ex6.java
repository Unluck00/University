/* 
Salvate tutto in un solo file Ex6.java.
Completate il metodo Tree <T> f(T x) in Leaf <T> e Branch <T> come segue.

1) f(x) rimuove tutti i nodi il cui elemento è uguale a x, sostituendoli con una foglia.
2) Se l'elemento di un nodo non è uguale a x, il metodo lascia il nodo invariato e procede ricorsivamente sui sottoalberi.
...
Eseguite il main della classe Ex6 come test per la risposta. Non definite il vostro metodo per casi sui test.
*/

import java.util.*;

abstract class Tree<T extends Comparable<T>> {
    public abstract Tree<T> f(T x);
...
class Ex6 {
    ...
    public static Tree<T> d(T elem, Tree<T> left, Tree<T> right) {
        if (elem.compareTo(x) == 0) return new Leaf<T>();
        else return new Branch<T>(elem, left, right);
    }
    ...
    public static void test() {
        ...
        check("t1 = " + t1.toString() + "\n t1.f(c)=", t1.f(x).toString(), ris1.toString());
        check("t2 = " + t2.toString() + "\n t2.f(c)=", t2.f(x).toString(), ris2.toString());
    }
    ...
    static void msg() {
        ...
        System.out.println("f rimuove i nodi con elemento uguale a c");
    }
