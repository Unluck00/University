/* 
Salvate tutto in un solo file Ex10.java.
Completate il metodo Tree <T> n(T x) in Leaf <T> e Branch <T> come segue.

1) n(x) aggiunge un nuovo nodo foglia come figlio sinistro di ogni nodo il cui elemento è maggiore di x.
...
Eseguite il main della classe Ex10 come test per la risposta. Non definite il vostro metodo per casi sui test.
*/

import java.util.*;

abstract class Tree<T extends Comparable<T>> {
    public abstract Tree<T> n(T x);
...
class Ex10 {
    ...
    public static Tree<T> d(T elem, Tree<T> left, Tree<T> right) {
        if (elem.compareTo(x) > 0) {
            return new Branch<T>(elem, new Leaf<T>(), right);
        } else {
            return new Branch<T>(elem, left, right);
        }
    }
    ...
    public static void test() {
        ...
        check("t1 = " + t1.toString() + "\n t1.n(b)=", t1.n(x).toString(), ris1.toString());
        check("t2 = " + t2.toString() + "\n t2.n(b)=", t2.n(x).toString(), ris2.toString());
    }
    ...
    static void msg() {
        ...
        System.out.println("n aggiunge una foglia come figlio sinistro dei nodi con elemento > b");
    }
