/* 
Salvate tutto in un solo file Ex8.java.
Completate il metodo Tree <T> h(T x, T y) in Leaf <T> e Branch <T> come segue.

1) h(x, y) sostituisce ogni occorrenza di x con y nell'albero.
2) Se un nodo non contiene x, il metodo lascia il nodo invariato e procede ricorsivamente sui sottoalberi.
3) Terminate restituendo l'indirizzo "this" dell'albero (l'indirizzo non cambia mai).
4) Usate a.compareTo(b) per confrontare a,b in T e supponete che gli elementi dell'albero e x,y non siano null.
5) Usate una definizione ricorsiva. Non modificate altro. Non usate "for" nè "while".

Eseguite il main della classe Ex8 come test per la risposta. Non definite il vostro metodo per casi sui test.
*/

import java.util.*;

abstract class Tree<T extends Comparable<T>> {
    public abstract Tree<T> h(T x, T y);
    public abstract String toString();
}

class Leaf<T extends Comparable<T>> extends Tree<T> {
    public Tree<T> h(T x, T y) {
...
class Ex8 {
    ...
    public static Tree<T> d(T elem, Tree<T> left, Tree<T> right) {
        if (elem.compareTo(x) == 0) {
            return new Branch<T>(y, left, right);
        } else {
            return new Branch<T>(elem, left, right);
        }
    }
    ...
    public static void test() {
        ...
        check("t1 = " + t1.toString() + "\n t1.h(b,z)=", t1.h(x, y).toString(), ris1.toString());
        check("t2 = " + t2.toString() + "\n t2.h(b,z)=", t2.h(x, y).toString(), ris2.toString());
    }
    ...
    static void msg() {
        ...
        System.out.println("h sostituisce ogni occorrenza di b con z");
    }
